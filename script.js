const state = {
    ...
    const sampleRows = [
      { name: "顧問服務", qty: 1, unit: 12000, priceType: "untaxed", taxType: "taxable" },
      { name: "課程設計", qty: 1, unit: 8000, priceType: "untaxed", taxType: "taxable" },
      { name: "教材製作", qty: 1, unit: 5000, priceType: "untaxed", taxType: "taxable" }
    ];
    ...
    function updatePrintPreview(rows, totals) {
      const activeRows = rows.filter((row) => row.unit > 0 && row.qty > 0);
      const invoiceType = document.getElementById("invoiceType").value;
      const invoiceDate = document.getElementById("invoiceDate").value || "未填";
      const invoiceNo = document.getElementById("invoiceNo").value.trim() || "未填";
      const buyerName = document.getElementById("buyerName").value.trim() || "未填";
      const buyerTaxId = document.getElementById("buyerTaxId").value.trim() || "未填";
      const buyerAddress = document.getElementById("buyerAddress").value.trim() || "未填";
      const memo = document.getElementById("invoiceMemo").value.trim() || "無";
      const taxRate = getTaxRate();
      const invoiceData = {
        activeRows,
        invoiceType,
      ...
      const rowHtml = rows.map((row) => {
        ...
        return `
          <tr>
            ...
            <td>${escapeHtml(data.memo === "無" ? "" : data.memo)}</td>
          </tr>
        `;
      }).join("");

      return `
        <div class="invoice-paper invoice-template">
          ...
          <div class="invoice-head-row">
            <div>
              買受人：<span class="invoice-fill">${data.invoiceType === "triplicate" ? escapeHtml(data.buyerName) : "非營業人"}</span>
            </div>
          ...
          <div class="invoice-head-row">
            <div class="invoice-tax-id-line">
              <span>統一編號：</span>
              ${renderTaxIdBoxes(data.invoiceType === "triplicate" ? data.buyerTaxId : "")}
            </div>
            <div></div>
          </div>
          <div class="invoice-address-line">
            <span>地址：</span>
            <div class="invoice-address-rule">${data.invoiceType === "triplicate" ? escapeHtml(data.buyerAddress) : ""}</div>
